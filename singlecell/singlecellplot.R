...
H2AFV = data.frame(cell = colnames(tpm), gene=log2(as.numeric(H2AFV)+1) )


H2AFZ = tpm[rownames(tpm)=="H2AFZ",]
H2AFZ = data.frame(cell = colnames(tpm), gene=log2(as.numeric(H2AFZ)+1) )

############################################################################################################

pdf("breast_primary_tumor_log2.pdf",width=14)

par(mfrow = c(2,1) )

stripchart(gene ~ cell, vertical = TRUE, data = H2AFV, jitter = 0.3, ylab = expression('Single Cell H2AFV log'[2]*' TPM'),
    method = "jitter", pch = 20, col = alpha(colour='red',alpha=.5),cex = 2)

stripchart(gene ~ cell, vertical = TRUE, data = H2AFZ, jitter = 0.3, ylab = expression('Single Cell H2AFV log'[2]*' TPM'),
    method = "jitter", pch = 20, col = alpha(colour='red',alpha=.5),cex = 2)
