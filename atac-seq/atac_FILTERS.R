...
write.table( x[x[,11]=="Up",], "atac_diffreps_w100_FDR5_50R_l2FC5_Up_FullStats.txt", sep="\t",quote=F,col.names=T,row.names=F)
write.table( x[x[,11]=="Down",], "atac_diffreps_w100_FDR5_50R_l2FC5_Down_FullStats.txt", sep="\t",quote=F,col.names=T,row.names=F)
##!R
##!SHELL
...
##!R
pdf("ATAC-up_pie.pdf")
res=read.table(pipe("more ATAC-up.annStats |cut -f1,2,4"), sep="\t",header=F)
i1 = which(res[,1]=="Annotation")[2]+1
i2 = dim(res)[1]
res = res[ i1:i2,]
tdown = as.numeric(as.character(res[,2]))
