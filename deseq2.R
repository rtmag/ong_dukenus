...
sig_vsd=dds_vsd[which(dds_res$padj<0.05 & abs(dds_res$log2FoldChange)>.6),]
...
pdf("heatmap_differentially_expressed_genes_log2fc.5.pdf")
...
pdf("volcano_plot_differentially_expressed_genes_log2fc.5.pdf")
...
points(dds_res$log2FoldChange[abs(dds_res$log2FoldChange)>.5 & dds_res$padj<0.05],
      ...
      col="red")
legend("topright", paste("shH2AFV:",length(which(dds_res$log2FoldChange>.5 & dds_res$padj<0.05))), bty="n") 
legend("topleft", paste("shNT:",length(which(dds_res$log2FoldChange<(-.5) & dds_res$padj<0.05))), bty="n") 
dev.off()

write.csv(dds_res[which(dds_res$padj<0.05 & abs(dds_res$log2FoldChange)>.5),],"differentially_expressed_genes_log2fc.5.csv")
